# using flowofpkts and flowofbytes
...
data=res.loc[:,["flowBytesPerSecond","flowPktsPerSecond","class1"]]

data["color"]=(data["class1"]).apply(lambda x: "blue" if x=="VPN" else "red")


plt.scatter(data["flowBytesPerSecond"],data["flowPktsPerSecond"],c=data["color"],alpha=0.5, s=12)
plt.xscale('log')
plt.yscale('log')
plt.title("Scatter plot for datapoints")
plt.xlabel("flowBytesPerSecond")
plt.ylabel("flowPktsPerSecond")
plt.show()

xtrain=data.loc[:,["flowBytesPerSecond","flowPktsPerSecond"]]
ytrain=data["class1"]
...
xtest=test_data.loc[:,["flowBytesPerSecond","flowPktsPerSecond"]]
...
plt.scatter(xtest["flowBytesPerSecond"],xtest["flowPktsPerSecond"],c=xtest["color"],alpha=0.5, s=12)
plt.xscale('log')
plt.yscale('log')
...
for i in range(4,11):
  ...
  pred=knn.predict(xtest.loc[:,["flowBytesPerSecond","flowPktsPerSecond"]])
